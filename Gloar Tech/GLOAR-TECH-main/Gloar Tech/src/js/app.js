...
async function loadHistorialVentas() {
  ...
  try {
    ...
    tb.innerHTML = vRes.data.map(v => {
      ...
      // con_itbis puede llegar como booleano, string "TRUE"/"FALSE", "true"/"false", 1/0
      const rawItbis = v.con_itbis;
      const conItbis = rawItbis === undefined || rawItbis === null
        ? true
        : rawItbis === true || rawItbis === 1 ||
      ...
      // Columna ITBIS: muestra monto si aplica, o "No" si no
      ...
      // Columna "s/ITBIS": siempre muestra el subtotal puro
      const sinItbisCell = Utils.fmt(subtotal);
