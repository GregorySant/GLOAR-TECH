...
function bindForms() {
  ...
  document.getElementById('listaCategorias')?.addEventListener('click', e => {
    const btn = e.target.closest('.cat-card__del');
    if (btn) handleDelCategoria(btn.dataset.catId, btn.dataset.catNombre);
  });
...
async function handleAddCategoria(e) {
  ...
  if (!nombre) { Utils.showAlert('statusCategoria', 'error', 'Escribe un nombre para la categoría.'); btn.disabled = false; return; }
  Utils.showAlert('statusCategoria', 'info', 'Agregando...');
  try {
    const res = await Api.addCategoria(nombre);
    if (res.status === 'success') {
...
async function handleDelCategoria(id, nombre) {
  if (!confirm(`¿Eliminar la categoría "${nombre}"?`)) return;
  Utils.showAlert('statusCategoria', 'info', 'Eliminando...');
  try {
    const res = await Api.delCategoria(id);
    if (res.status === 'success') {
      Utils.showAlert('statusCategoria', 'success', `Categoría "${nombre}" eliminada.`);
      loadInitialData();
    } else Utils.showAlert('statusCategoria', 'error', res.message);
  } catch (err) { Utils.showAlert('statusCategoria', 'error', err.message); }
}
