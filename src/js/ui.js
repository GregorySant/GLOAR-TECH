...
const UI = (() => {
  ...
  function buildCategoryList(cats) {
    ...
    if (!container) return;
    if (cats.length === 0) {
      container.innerHTML = '<p class="cat-empty"><i class="fas fa-tags"></i><br>No hay categorías aún. Agrega la primera arriba.</p>';
      return;
    }
    container.innerHTML = cats.map(c => `
      <div class="cat-card">
        <div>
          <div class="cat-card__name"><i class="fas fa-tag" style="color:var(--color-primary);margin-right:6px;"></i>${c.nombre}</div>
          <div class="cat-card__id">ID: ${c.id}</div>
        </div>
        <button class="cat-card__del" title="Eliminar categoría" data-cat-id="${c.id}" data-cat-nombre="${c.nombre}">
